$('.payment-radio').on('keyup click', function(){ $('.phoneinput1, .phoneinput2').hide(); $('.amountselect').show(); });
$('.payment-radio.phonemobile').on('keyup click', function(){ $('.amountselect, .phoneinput2').hide(); $('.phoneinput1').show(); });
$('.payment-radio.phoneqiwi').on('keyup click', function(){ $('.amountselect, .phoneinput1').hide(); $('.phoneinput2').show(); });
$('.amount-radio').on('keyup click', function(){ selectamount=$('input[name=selectamount]:checked').val(); $('#amount').val(selectamount); });
$('#phonemobile, #phoneqiwi').on('keyup change input click', function(){
if(this.value.match(/[^0-9.]/g)){ this.value=this.value.replace(/[^0-9.]/g, '');} });


$('#amount').on('keyup change input click', function(){ mindep=Number(min);
if(getCookie('lng')=='EN'){ deppperr='<b>Minimum amount '+valll+mindep+'!</b>';
...
if(getCookie('lng')==null){ deppperr='<b>Минимальная сумма '+valll+mindep+'!</b>';
...
amount=Number($('#amount').val()); if(mindep>amount){
$('#depbutton').attr('disabled',true).css({'opacity':'0.7'});
$('.deptext').html(deppperr);return false;}else{
...
desum=$('#amount').val();
phone=$('#phoneqiwi').val();
if(phone==''){ phone=$('#phonemobile').val(); }
if((deway=='qiwi_rub') || (deway=='qiwi_usd') || (deway=='qiwi_eur')){
if(phone.length<11){ $('.deptext').html(deppperr3); return; } }
if((deway=='mts_rub') || (deway=='beeline_rub') || (deway=='tele2_rub') || (deway=='megafon_rub')){
if(phone.length<10){ $('.deptext').html(deppperr4); return; } }
if(phone==''){ $('.depwindow').load('/deposit/pay/?payway='+deway+'&amount='+desum);
errrr=$('.depwindow').text();
if(errrr==1){ $('.deptext').html(deppperr4); }
if(errrr==2){ $('.deptext').html(deppperr3); }
if(errrr==3){ $('.deptext').html(deppperr6); }
if(errrr==4){ $('.deptext').html(deppperr5); } }else{
$('.depwindow').load('/deposit/pay/?payway='+deway+'&amount='+desum+'&phone='+phone);
errrr=$('.depwindow').text();
if((deway=='mts_rub') || (deway=='beeline_rub') || (deway=='tele2_rub') || (deway=='megafon_rub')){
$('.deptext').html(deppperr7);
if(errrr==5){ $('.deptext').html(deppperr7); } }
if(errrr==1){ $('.deptext').html(deppperr3); }
if(errrr==2){ $('.deptext').html(deppperr4); }
if(errrr==3){ $('.deptext').html(deppperr6); }
if(errrr==4){ $('.deptext').html(deppperr5); } } return false; });
...
$('.marker2.onPhone').on('keyup click', function(){ $('#reg_ema').val(''); });
$('.marker2.onEmail').on('keyup click', function(){ $('#reg_phn').val(''); });
$('#gift1').on('keyup click', function(){ $('#reg_gif').val('1'); });
$('#gift2').on('keyup click', function(){ $('#reg_gif').val('2'); });
$('#gift3').on('keyup click', function(){ $('#reg_gif').val('3'); });



$(function(){$('#searchquery').bind('change keyup input click',
function(){if(this.value.length >= 2){
$.ajax({
type: 'post',url: '/core/search/',
data: {'searchquery':this.value},
cache: false,
success: function(html){ $(".games").html(html); }
})}})});

$(function(){$('#brandquery').bind('change select',
function(){if(this.value.length==2){
$.ajax({
